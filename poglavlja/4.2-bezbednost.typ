...
_Nginx_ у систему има улогу _reverse proxy_ сервера @nginx_reverse_proxy који усмерава све спољне захтеве ка одговарајућим сервисима унутар _Docker_ окружења. Поред усмеравања, он обезбеђује и више безбедносних механизама:
 - контролу приступа,
...
Захтеви ка складишту података (_MinIO_) пролазе кроз посебан механизам аутентификације који користи _auth_request_ директиву @nginx_auth_request. Сваки захтев се најпре прослеђује _Django_ серверу ради валидације токена. Уколико је приступ дозвољен, пропушта ка _MinIO_ сервису. На овај начин се спречава неовлашћени приступ фајловима.

Поред HTTP и _WebSocket_ комуникације, _Nginx_ такође обрађује SSH саобраћај ка систему за верзионисање (_Gitea_) преко _stream_ модула, где делује као TCP proxy и преусмерава захтеве са порта 2222 на унутрашњи порт 22 @nginx_stream.
